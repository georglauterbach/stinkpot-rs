...
session = excluded.session",
    ...
    fn import(history_file: Option<&path::PathBuf>) -> Result<()> {
        ...
        let mut shell_commands_count = 0;
...
impl SearchApp {
    ...
    fn accept(&mut self) {
        if let Some(i) = self.list_state.selected()
            && let Some(&idx) = self.filtered.get(i)
        {
            self.selected = Some(self.all[idx].cmd.clone());
        }
    }
}
...
impl Tui {
    ...
    fn draw_search(f: &mut ratatui::Frame, app: &SearchApp) {
        ...
        let cursor_x = u16::try_from(2 + app.cursor_col).unwrap_or(u16::MAX);
        ...
        let max_rows = max_rows.parse::<usize>().unwrap_or(12);

        let start = sel.saturating_sub(max_rows - 1);
        let end = (start + max_rows).min(app.filtered.len());

        let mut ts_width = 0usize;
        let times: Vec<String> = (start..end)
            .map(|i| {
                let t = Time::relative_short(app.all[app.filtered[i]].ts);
        ...
        let items: Vec<widgets::ListItem> = (start..end)
            .map(|i| {
                let c = &app.all[app.filtered[i]];
                let line = c.cmd.replace('\n', "  ");
                let ts = format!("{:>width$}", times[i - start], width = ts_width);
        ...
        if let Some(s) = app.list_state.selected()
            ...
            && s < end
        {
            window_state.select(Some(s - start));
        }
...
mod tests {
    ...
    fn load_candidates_newest_first() {
        ...
        let cands = Database::load_candidates(&conn).expect("load candidates");
        assert_eq!(
            cands.iter().map(|c| c.cmd.as_str()).collect::<Vec<_>>(),
            ["new", "mid", "old"]
        );
        let _ = fs::remove_dir_all(&dir);
    }
    ...
    fn search_app_filters_and_accepts() {
        ...
        assert!(app.filtered.iter().all(|&i| app.all[i].cmd.contains("git")));
        app.accept();
